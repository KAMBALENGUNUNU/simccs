...
public class AuthController {
        ...
        public ResponseEntity<ResponseDTO<JwtResponse>> authenticateUser(
                ...
                if (!userDetails.isEnabled()) {
                        return ResponseDTO
                                        .<JwtResponse>error(HttpStatus.FORBIDDEN.value(),
                                                        "Error: Account is not approved by Admin yet.")
                                        .toResponseEntity();
                }

                if (userDetails.isMfaEnabled()) {
                        String mfaToken = jwtService.generateMfaToken(userDetails.getUsername());
                        ResponseCookie mfaCookie = ResponseCookie.from("mfa_session", mfaToken)
                                        .httpOnly(true)
                        ...
                        return ResponseEntity.status(HttpStatus.FORBIDDEN)
                                        ...
                                        .body(ResponseDTO.<JwtResponse>error(HttpStatus.FORBIDDEN.value(),
                                                        "MFA_REQUIRED"));
                }
        ...
        public ResponseEntity<ResponseDTO<MessageResponse>> registerUser(
                ...
                User user = new User(signUpRequest.getEmail(), encoder.encode(signUpRequest.getPassword()),
                                signUpRequest.getFullName(), signUpRequest.getNationalId());
                ...
                if (strRoles == null) {
                        ...
                        strRoles.forEach(role -> {
                                switch (role) {
                                        // FIX: Matching standard Spring Security naming convention used by frontend
                                        // Enums
                                        case "ROLE_ADMIN" ->
                                                roles.add(roleRepository.findByName(ERole.ROLE_ADMIN).orElseThrow());
                                        case "ROLE_EDITOR" ->
                                                roles.add(roleRepository.findByName(ERole.ROLE_EDITOR).orElseThrow());
                                        default -> roles.add(
                                                        roleRepository.findByName(ERole.ROLE_JOURNALIST).orElseThrow());
                                }
                        });
                }
                ...
                // --- NOTIFICATION ---
                notificationService.notifyAdminOfNewUser(user.getEmail(), user.getFullName(), user.getNationalId());

                return ResponseDTO
                                .success("User registered successfully! Wait for Admin approval.",
                                                new MessageResponse("Registered"))
                                .toResponseEntity();
        }
}